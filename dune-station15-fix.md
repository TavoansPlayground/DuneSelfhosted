# Fix: Station 15 (CB_SurvivalChallenge_Station_15) Not Loading on Self-Hosted Server

## Symptom

Players cannot enter **Station 15** from the Overland map. The map never loads, or
the server spins up and exits ~5 seconds after becoming ready.

## Root Cause

Station 15 maps to the internal name **`CB_SurvivalChallenge_Station_15`** (display
label: `Station15`). It was added to the `initialize_partitions_full_battlegroup()`
stored function by Funcom's `DA-11609` upgrade script, but the operator does not
re-run partition initialisation for an existing battlegroup, so the partition stays
absent until the CRD is patched manually.

Confirmed via Overland server travel-destination log:
```
Register TravelDestination(Overland_to_Station15) for Map(Station15)
```
...
kubectl exec -n $NS \
  ...
  -c "SELECT map, label FROM world_partition WHERE map='CB_SurvivalChallenge_Station_15'"
```
...
Use `result + 1` as the `id` value in Step 3 (standard install with all other
fixes applied = **31**).
...
kubectl patch battlegroup $BG -n $NS --type=json -p "[
  {
    ...
    \"value\": {
      \"map\": \"CB_SurvivalChallenge_Station_15\",
      \"partitions\": [{\"dimension\": 0, \"disable\": false, \"id\": 31,
...
if "CB_SurvivalChallenge_Station_15" in ini:
    print("Already present — nothing to do.")
else:
    new_ini = ini + "\n\n[ CB_SurvivalChallenge_Station_15 ]\nNumExtraServers = 0"
    patch = json.dumps([{
        "op": "replace",
...
kubectl exec -n $NS \
  $(kubectl get pod -n $NS -l igw.funcom.com/role=database \
    -o jsonpath='{.items[0].metadata.name}') -- \
  psql -U dune -d dune -h 127.0.0.1 -p 15432 -t \
  -c "SELECT partition_id, map, label FROM world_partition WHERE map='CB_SurvivalChallenge_Station_15'"
```

Expected: `31 | CB_SurvivalChallenge_Station_15 | Station15_0`

## Expected behaviour after the fix

1. Player uses the Overland portal to Station 15.
2. Gateway finds partition 31 (`Station15_0`).
3. Director spawns a `CB_SurvivalChallenge_Station_15` server pod (~30–60 s cold start
   on first visit).
